...
from .utils.backend import ascend_setup, ascend_build
...
class AscendCEvaluator:
    ...
    def compile(
        ...
        result = ascend_build(
            ...
            full_code=full_code,
            verbose=self.verbose,
            skip_model_exec=True,  # sandbox workers handle exec after setting env vars
        )
    ...
    def _setup_opp_environment(self, project_path: str, op_name: str) -> None:
        ...
        # Disable CANN exception dump to avoid generating large exception_info files
        os.environ.setdefault("ASCEND_AICPU_EXCEPTION_DUMP", "0")

        project_dir = Path(project_path)
        op_capital = op_name[0].upper() + op_name[1:] + "Custom"
        # Must match ascend_compile.py and sandbox.py:
        # .run installer deploys to opp/vendors/customize/
        custom_opp_path = project_dir / "opp" / "vendors" / "customize"
    ...
    def verify_correctness(
        ...
        Args:
            ...
            device: NPU device string (e.g., "npu:0", "npu:3")
    ...
    def measure_performance(
        self,
        op_name: str,  # noqa: ARG002
        python_reference: Optional[str] = None,
        device: str = "npu:0",
    ) -> Dict[str, Any]:
        """
    ...
    def verify_and_measure(
        ...
        """
        Combined correctness + performance in a single sandbox subprocess.

        Saves ~10-20s by avoiding redundant NPU initialization.
        ...
        return self._sandbox.verify_and_measure_sandbox(
            python_reference=python_reference,
            context_data=context_data,
            device=device,
            num_correctness_trials=self.num_correctness_trials,
            seed=self.seed,
            num_warmup=self.num_warmup,
