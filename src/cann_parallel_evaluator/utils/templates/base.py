...
class TemplateBase:
    ...
    def __init__(self, signature: Dict[str, Any]):
        ...
        # We must match: MatrixMultiplicationCustom -> matrix_multiplication_custom
        #                Avgpooling2dCustom -> avgpooling2d_custom
        self.op_custom_capital = self._to_pascal_case(f"{self.op_name_lower}_custom")
        self.op_custom = re.sub(r'(?<!^)(?=[A-Z])', '_', self.op_custom_capital).lower()
    ...
    def _dtype_to_cpp_type(self, dtype: str) -> str:
        ...
        dtype_map = {
            ...
            "int64": "int64_t",
            "bool": "bool",
            "list_int": "at::IntArrayRef",
            "list_float": "at::ArrayRef<float>",
        }
    ...
    def _dtype_to_ge_datatype(self, dtype: str) -> str:
        ...
        """
        return self._GE_DTYPE_MAP.get(dtype.lower(), "ge::DT_FLOAT")

    def _dtypes_to_ge_datatypes(self, dtype: Union[str, List[str]]) -> str:
        """Convert one or more dtypes to a comma-separated ge::DataType string.

        Supports both single dtype and multi-dtype:
            "float"              -> "ge::DT_FLOAT"
            ["float16", "float"] -> "ge::DT_FLOAT16, ge::DT_FLOAT"
        """
        if isinstance(dtype, str):
            return self._dtype_to_ge_datatype(dtype)
        # list of dtypes — deduplicate while preserving order
        seen = set()
        ge_types = []
        for d in dtype:
            ge = self._dtype_to_ge_datatype(d)
            if ge not in seen:
                seen.add(ge)
                ge_types.append(ge)
        return ", ".join(ge_types)
