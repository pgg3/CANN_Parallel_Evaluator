    ...
    def generate(
        ...
        for inp in inputs:
            if inp.get("is_tensor", True):
                ge_dtypes = self._dtypes_to_ge_datatypes(inp.get("dtype", "float"))
                input_defs += f'        this->Input("{inp["name"]}").ParamType(REQUIRED).DataType({{{ge_dtypes}}}).Format({{ge::FORMAT_ND}});\n'
            else:
                attr_defs += self._gen_attr_def(inp)

        for param in init_params:
            if param.get("is_tensor", False):
                ge_dtypes = self._dtypes_to_ge_datatypes(param.get("dtype", "float"))
                input_defs += f'        this->Input("{param["name"]}").ParamType(REQUIRED).DataType({{{ge_dtypes}}}).Format({{ge::FORMAT_ND}});\n'
            else:
        ...
        for out in outputs:
            if out.get("is_tensor", True):
                ge_dtypes = self._dtypes_to_ge_datatypes(out.get("dtype", "float"))
                output_defs += f'        this->Output("{out["name"]}").ParamType(REQUIRED).DataType({{{ge_dtypes}}}).Format({{ge::FORMAT_ND}});\n'
        ...
        # Extract #include lines from tiling_func_body and hoist to file top
        extra_includes = ""
        clean_body_lines = []
        for line in tiling_func_body.split("\n"):
            stripped = line.strip()
            if stripped.startswith("#include"):
                extra_includes += stripped + "\n"
            else:
...
'''
    ...
    def _gen_attr_def(self, param: Dict[str, Any]) -> str:
        ...
        elif dtype_lower == "list_int":
            if has_default:
                default_str = "{" + ", ".join(str(x) for x in default_val) + "}"
                return f'        this->Attr("{name}").AttrType({attr_type}).ListInt({default_str});\n'
            return f'        this->Attr("{name}").AttrType({attr_type}).ListInt();\n'
        elif dtype_lower == "list_float":
            if has_default:
                default_str = "{" + ", ".join(str(x) for x in default_val) + "}"
                return f'        this->Attr("{name}").AttrType({attr_type}).ListFloat({default_str});\n'
            return f'        this->Attr("{name}").AttrType({attr_type}).ListFloat();\n'
        else:
            if has_default:
                return f'        this->Attr("{name}").AttrType({attr_type}).Float({default_val});\n'
            return f'        this->Attr("{name}").AttrType({attr_type}).Float();\n'
